	...
	switch ( description.type ) {
		case 'plane':
			...
			if ( ( shape = getShapeFromCache( cache_key ) ) === null ) {
				...
				shape = new a.btStaticPlaneShape(_vec3_1, 0 );
		...
		case 'box':
			...
			if ( ( shape = getShapeFromCache( cache_key ) ) === null ) {
				...
				shape = new a.btBoxShape(_vec3_1);
				setShapeCache( cache_key, shape );
			}
			break;
		...
		case 'sphere':
			...
			if ( ( shape = getShapeFromCache( cache_key ) ) === null ) {
				shape = new a.btSphereShape( description.radius );
				setShapeCache( cache_key, shape );
			}
			break;
		...
		case 'cylinder':
			...
			if ( ( shape = getShapeFromCache( cache_key ) ) === null ) {
				...
				_vec3_1.setY(description.height / 2);
				_vec3_1.setZ(description.depth / 2);
				shape = new a.btCylinderShape(_vec3_1);
				setShapeCache( cache_key, shape );
			}
			break;
		...
		case 'capsule':
			...
			if ( ( shape = getShapeFromCache( cache_key ) ) === null ) {
				...
				shape = new a.btCapsuleShape( description.radius, description.height - 2 * description.radius );
				setShapeCache( cache_key, shape );
			}
			break;
		...
		case 'cone':
			...
			if ( ( shape = getShapeFromCache( cache_key ) ) === null ) {
				shape = new a.btConeShape( description.radius, description.height );
				setShapeCache( cache_key, shape );
			}
			break;

		case 'concave':
			var i, triangle, triangle_mesh = new a.btTriangleMesh;
			...
			shape = new a.btBvhTriangleMeshShape(
				triangle_mesh,
				true,
				true
			);
		...
		case 'convex':
			var i, point, shape = new a.btConvexHullShape;
		...
		case 'heightfield':
			...
			var ptr = a.allocate(4 * description.xpts * description.ypts, "float", a.ALLOC_NORMAL);

			for (var f = 0; f < description.points.length; f++) {
				a.setValue(ptr + f,  description.points[f]  , 'float');
			}

			shape = new a.btHeightfieldTerrainShape(
					description.xpts,
					description.ypts,
					ptr,
					1,
...
public_functions.init = function( params ) {
  ...
  _transform = new a.btTransform();
	_vec3_1 = new a.btVector3(0,0,0);
	_vec3_2 = new a.btVector3(0,0,0);
	_vec3_3 = new a.btVector3(0,0,0);
	_quat = new a.btQuaternion(0,0,0,0);
	...
	var collisionConfiguration = new a.btDefaultCollisionConfiguration,
		dispatcher = new a.btCollisionDispatcher( collisionConfiguration ),
	...
	world = new a.btDiscreteDynamicsWorld( dispatcher, broadphase, solver, collisionConfiguration );
...
if ( description.children ) {
	...
	motionState = new a.btDefaultMotionState( _transform ); // #TODO: btDefaultMotionState supports center of mass offset as second argument - implement
	rbInfo = new a.btRigidBodyConstructionInfo( description.mass, motionState, shape, _vec3_1 );
...
public_functions.addVehicle = function( description ) {
	var vehicle_tuning = new a.btVehicleTuning(),
		vehicle;
	...
	vehicle = new a.btRaycastVehicle( vehicle_tuning, _objects[ description.rigidBody ], new a.btDefaultVehicleRaycaster( world ) );
...
public_functions.removeObject = function( details ) {
	...
	a.destroy(_objects[details.id]);
	a.destroy(_motion_states[details.id]);
    if (_compound_shapes[details.id]) a.destroy(_compound_shapes[details.id]);
	if (_noncached_shapes[details.id]) a.destroy(_noncached_shapes[details.id]);
...
public_functions.addConstraint = function ( details ) {
	...
	switch ( details.type ) {
		...
		case 'slider':
			...
			a.destroy(transforma);
			if (transformb != undefined) {
				a.destroy(transformb);
			}
			break;
		...
		case 'conetwist':
			...
			constraint = new a.btConeTwistConstraint(
				_objects[ details.objecta ],
				_objects[ details.objectb ],
				transforma,
				transformb
