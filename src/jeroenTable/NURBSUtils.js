...
export default {
  ...
  */
  calcBasisFunctionDerivatives(span, u, p, n, U) {
    const zeroArr = [];
    for (let i = 0; i <= p; i += 1) { zeroArr[i] = 0.0; }

    const ders = [];
    for (let i = 0; i <= n; i += 1) { ders[i] = zeroArr.slice(0); }

    const ndu = [];
    for (let i = 0; i <= p; i += 1) { ndu[i] = zeroArr.slice(0); }
